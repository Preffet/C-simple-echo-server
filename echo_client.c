...
int main(int argc, char *argv[]) {
    ...
    // set server address using inet_aton (which converts address from the IPv4 numbers-and-dots notation into binary form)
    ...
    while (echo_buf[0] != '.') {
        ...
        ssize_t bytesReply = recvfrom(sockfd, reply_buf, 4, 0, (struct sockaddr*) &server, &len);

        if(bytesReply==-1||bytesReply==0){ printf("server failed to reply\n");}
